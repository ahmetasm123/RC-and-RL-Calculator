# -*- coding: utf-8 -*-
# Adding encoding declaration for unicode characters like Ω, µ, ∠ etc.
...
# --- Calculation Core ---
# (Keep calculate_derived_reactance_params and calculate_series_ac_circuit as before)
def calculate_derived_reactance_params(
    ...
    """
    Calculates the missing parameter (Reactance, Component Value, or Omega)
    given two others for RL or RC circuits. Also performs consistency checks.
    (Implementation details omitted for brevity - assume it's the same as the previous working version)
    """
    # Validate inputs slightly (more checks in calling functions)
    ...
    if param_type == 'C' and component_val is not None and component_val <= 0: # Allow None, but not <= 0
    ...
    if inputs_provided < 2:
        # Not an error yet, just return Nones, calling function checks sufficiency
        return component_val, reactance_val, omega

    # --- Derive the missing third parameter ---
    ...
    try:
        if omega is not None and component_val is not None and reactance_val is None:
            if omega == 0:
                ...
                if component_val == 0: # Should be caught above, but safeguard
            ...
            if omega == 0:
                if param_type == 'L':
                    if reactance_val == 0:
                        # L is indeterminate if omega=0 and X_L=0
                        derived_component = None # Cannot determine L
                    else:
                        raise ValueError("Inconsistency: X_L must be 0 if frequency is 0 Hz.")
                else: # param_type == 'C'
                    # Cannot determine C if omega=0 (X_C would be inf)
                     raise ValueError(f"Cannot determine C from X_C ({reactance_val} Ω) if frequency is 0 Hz.")
            elif param_type == 'L':
                 if omega == 0: # Should be caught above, but safeguard
            ...
            if param_type == 'L':
                if component_val == 0:
                    if reactance_val == 0:
                        # Omega indeterminate if L=0 and X_L=0
                        derived_omega = None
                    else:
                ...
                if component_val <= 0: # Check again
                     raise ValueError("Capacitance must be positive.")
                if reactance_val == 0:
                    raise ValueError("Cannot calculate frequency if X_C is 0 (implies infinite frequency or C=inf).")
                if reactance_val == float('inf'):
                     derived_omega = 0.0 # DC case
                else:
                     derived_omega = 1.0 / (component_val * reactance_val)

    except ZeroDivisionError:
        # Should be caught by specific checks, but as a safeguard
        raise ValueError("Calculation error: Division by zero occurred.")


    # --- Consistency Check (if all 3 were available initially or derived) ---
    ...
    if all(x is not None for x in final_vals):
        ...
        # Check component value validity again before using in calculation
        if param_type == 'C' and (c is None or c <= 0):
             pass # Cannot calculate expected reactance if C is invalid
        elif param_type == 'L' and c is None:
             pass # Cannot calculate expected reactance if L is invalid
        elif w == 0:
        ...
        # Use math.isclose for floating point comparison, handle inf case
        if expected_reactance is not None:
            # Check if r is finite/infinite before comparing
            r_is_finite = not math.isinf(r)
            exp_is_finite = not math.isinf(expected_reactance)

            if r_is_finite != exp_is_finite: # One is inf, other is not
                 consistent = False
            elif not r_is_finite: # Both are inf
                 consistent = True
            ...
            if not consistent:
                ...
                # Format values carefully for error message
                r_str = f"{r:.4g}" if r != float('inf') else "Infinity"
                exp_str = f"{expected_reactance:.4g}" if expected_reactance != float('inf') else "Infinity"
                raise ValueError(
                    f"Inconsistent input: Provided/derived {param_name} ({r_str} Ω) "
                    f"does not match calculated value ({exp_str} Ω) "
                    f"from {comp_name} and frequency."
                )
...
def calculate_series_ac_circuit(
    V_rms: float, R: float,
    component_val: Optional[float], # L or C
    reactance_val: Optional[float], # X_L or X_C
    f: Optional[float],
    circuit_type: str # 'RL' or 'RC'
) -> Dict[str, Optional[float]]:
    """
    Calculates parameters for a series RL or RC AC circuit.
    (Implementation details omitted for brevity - assume it's the same as the previous working version)
    """
    # --- Input Validation ---
    if V_rms < 0: raise ValueError("V RMS must be non-negative.")
    if R < 0: raise ValueError("Resistance (R) must be non-negative.")
    if f is not None and f < 0: raise ValueError("Frequency (f) must be non-negative.")
    # Component/Reactance specific checks done in calculate_derived_reactance_params

    param_type = 'L' if circuit_type == 'RL' else 'C'
    omega = TWO_PI * f if f is not None else None

    # --- Derive missing parameter & Check Consistency ---
    try:
    ...
    # --- Check Sufficiency ---
    # Need R, V_rms, and enough info to determine reactance and omega
    # We need a definite reactance value.
    # We also need omega, unless it's a DC open circuit (X_C=inf, omega=0 implied)
    ...
    if final_reactance is None or (final_omega is None and not is_dc_open_circuit):
        raise ValueError(f"Insufficient parameters for {circuit_type} circuit. "
                         f"Need V_rms, R, and two of ({param_type}, X_{param_type}, f).")

    # Infer final frequency if possible
    if final_omega is not None:
        final_f = final_omega / TWO_PI
    elif is_dc_open_circuit:
         final_f = 0.0 # Implied frequency for DC open circuit
         final_omega = 0.0 # Ensure omega is 0 for DC case results
    else:
         final_f = None # Cannot determine frequency


    # --- Calculate Common Parameters ---
    is_rc = (circuit_type == 'RC')
    # Use signed reactance for phase calculation: +X_L, -X_C
    # Handle potential None for final_reactance (shouldn't happen if sufficiency check passed)
    if final_reactance is None:
         raise ValueError("Internal Error: Reactance could not be determined.") # Safeguard

    reactance_signed = final_reactance if not is_rc else -final_reactance

    # Handle infinite reactance (DC capacitor)
    if final_reactance == float('inf'):
        Z = float('inf')
        phi = -90.0  # Current leads voltage by 90 deg (impedance angle)
        I_rms = 0.0
        V_rms_R = 0.0
        V_rms_X = V_rms # Capacitor voltage equals source voltage
    # Handle zero impedance (R=0 and X=0)
    elif R == 0 and final_reactance == 0:
        Z = 0.0
        phi = 0.0
        # Current is infinite if V_rms > 0
        if V_rms > 0:
             I_rms = float('inf')
             V_rms_R = 0.0 # I * 0
             V_rms_X = 0.0 # I * 0
        else: # V_rms = 0
             I_rms = 0.0 # Or indeterminate? Assume 0.
             V_rms_R = 0.0
             V_rms_X = 0.0
    else:
        # Normal Calculation
        ...
        # Phase Angle (of Impedance Z = R + jX_signed): atan2(X_signed, R)
        phi = math.degrees(math.atan2(reactance_signed, R))

        # Current: I = V / Z
        if Z == 0: # Should only happen if R=0, X=0 which is handled above
             I_rms = float('inf') if V_rms > 0 else 0.0
        else:
             I_rms = V_rms / Z

        # Component Voltages
        V_rms_R = I_rms * R if I_rms != float('inf') else (float('inf') if R > 0 and V_rms > 0 else 0.0)
        V_rms_X = I_rms * final_reactance if I_rms != float('inf') else (float('inf') if final_reactance > 0 and V_rms > 0 else 0.0)
    ...
    results = {
        ...
        'f': final_f,
        'omega': final_omega,
        param_type: final_comp,         # L or C
        f'X_{param_type}': final_reactance, # X_L or X_C
        'X': final_reactance,           # Generic Reactance Magnitude
        'Z': Z,                         # Impedance Magnitude
        'phi': phi,                     # Impedance Phase Angle (degrees)
        'I_rms': I_rms,
        'I_peak': I_peak,
        'V_rms_R': V_rms_R,
        'V_rms_X': V_rms_X,             # Voltage across Reactance (magnitude)
        'V_peak': V_peak,
        # Store original inputs for calculation steps display
...
def plot_waveforms(params: Dict[str, Optional[float]], circuit_type: str):
    ...
    if is_dc:
        ...
        if missing:
            raise ValueError(f'Missing required parameters for DC plotting: {", ".join(missing)}')
    else: # AC case
        missing = [key for key in required_keys_ac if params.get(key) is None]
        if missing:
            raise ValueError(f'Missing required parameters for AC plotting: {", ".join(missing)}')
        if params.get('omega', 0) <= 0: # Check omega specifically for AC
             raise ValueError("Cannot plot AC waveform without a positive angular frequency (omega).")
    ...
    if params.get('Z') == float('inf'):
         messagebox.showinfo("Plot Info", "Cannot plot waveforms for open circuit (infinite impedance, zero current).")
         return
    if I_peak == float('inf'):
         messagebox.showinfo("Plot Info", "Cannot plot waveforms for short circuit (infinite current).")
         return
    if V_peak == 0 and I_peak == 0:
        messagebox.showinfo("Plot Info", "Input voltage is zero, waveforms are all zero.")
        plt.figure(figsize=(10, 6))
        plt.axhline(0, color='black', linewidth=0.8)
        plt.title(f'{circuit_type} Circuit - Zero Input')
        plt.xlabel('Time (s)'); plt.ylabel('Amplitude (V or A)'); plt.grid(True);
    ...
    # --- Handle DC case (f=0) ---
    if is_dc:
        messagebox.showinfo("Plot Info", "Plotting waveforms for DC (0 Hz). Showing constant values.")
        ...
        plt.figure(figsize=(10, 6))
        plt.plot(t, np.full_like(t, v_s_val), label=f'Source Voltage = {v_s_val:.4g} V', marker='.')
        plt.plot(t, np.full_like(t, i_val), label=f'Circuit Current = {i_val:.4g} A', marker='.')
        plt.plot(t, np.full_like(t, vr_val), label=f'Voltage across R = {vr_val:.4g} V', marker='.')
        comp_label = 'L (Short)' if circuit_type == 'RL' else 'C (Open)'
        plt.plot(t, np.full_like(t, vx_val), label=f'Voltage across {comp_label} = {vx_val:.4g} V', marker='.')
        ...
        all_vals = [v for v in [v_s_val, i_val, vr_val, vx_val] if v is not None and not math.isinf(v)]
    ...
    plt.plot(t, v_s, label=f'Source Voltage ($V_S$)', linewidth=1.5)
    plt.plot(t, i, label=f'Circuit Current ($I$)', linewidth=1.5, linestyle='--')
    plt.plot(t, v_R, label=f'Voltage across R ($V_R$)', linewidth=1.2)
    comp_label = 'L' if circuit_type == 'RL' else 'C'
    plt.plot(t, v_X, label=f'Voltage across {comp_label} ($V_{comp_label}$)', linewidth=1.2)
...
def plot_phasors(params: Dict[str, Optional[float]], circuit_type: str):
    ...
    if missing:
        raise ValueError(f'Missing required parameters for phasor plot: {", ".join(missing)}')

    # Phasors are generally not meaningful for DC (f=0)
    f_val = params['f']
    if f_val is not None and math.isclose(f_val, 0):
        messagebox.showinfo("Phasor Plot Info", "Phasor diagrams are typically used for AC circuits (f > 0).")
        return

    # Check for infinite impedance/current
    if params.get('Z') == float('inf') or params.get('I_rms') == float('inf'):
        messagebox.showinfo("Phasor Plot Info", "Cannot plot phasors for open or short circuits.")
        return
    ...
    for label, (mag, angle_rad, color, width) in phasors.items():
        ...
        ax.quiver(0, 0, x_end, y_end, angles='xy', scale_units='xy', scale=1, color=color, width=0.005 * width, label=f'{label} ({mag:.3g})')
...
class ACCircuitSolverApp:
    ...
    def __init__(self, master: tk.Tk):
        ...
        style.configure('Header.TLabel', font=('Segoe UI', 10, 'bold')) # Style for headers
    ...
    def _create_widgets(self):
        ...
        sel_frame.grid(row=current_row, column=0, columnspan=3, sticky=tk.W, pady=(0, 10))
        ttk.Label(sel_frame, text='Circuit Type:', style='Header.TLabel').pack(side=tk.LEFT, padx=(0, 10))
        rl_rb = ttk.Radiobutton(sel_frame, text='RL', variable=self.circuit_var, value='RL', command=self._toggle_fields)
        rl_rb.pack(side=tk.LEFT, padx=5)
        rc_rb = ttk.Radiobutton(sel_frame, text='RC', variable=self.circuit_var, value='RC', command=self._toggle_fields)
        rc_rb.pack(side=tk.LEFT, padx=5)
        current_row += 1

        # --- Input Fields ---
        input_labelframe = ttk.LabelFrame(self.frame, text=" Inputs (* Required, [Opt] Optional) ", padding=(10, 5))
        input_labelframe.grid(row=current_row, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=(0, 5))
        input_labelframe.columnconfigure(1, weight=1) # Make entry column inside frame expand
        current_row += 1
        input_row_idx = 0

        # Create input rows using helper
        self.widgets['V'] = self._make_input_row(input_labelframe, input_row_idx, 'V RMS', 'V', list(UNIT_FACTORS['V'].keys()), is_optional=False)
        input_row_idx += 1
        self.widgets['R'] = self._make_input_row(input_labelframe, input_row_idx, 'R', 'Ω', list(UNIT_FACTORS['R'].keys()), is_optional=False)
        input_row_idx += 1
        self.widgets['f'] = self._make_input_row(input_labelframe, input_row_idx, 'Freq', 'Hz', list(UNIT_FACTORS['f'].keys()), is_optional=True)
        input_row_idx += 1
        self.widgets['L'] = self._make_input_row(input_labelframe, input_row_idx, 'L', 'mH', list(UNIT_FACTORS['L'].keys()), is_optional=True)
        self.rl_widgets_ref.extend(self.widgets['L'].values())
        input_row_idx += 1
        self.widgets['XL'] = self._make_input_row(input_labelframe, input_row_idx, 'X\u2097', 'Ω', list(UNIT_FACTORS['X'].keys()), is_optional=True)
        self.rl_widgets_ref.extend(self.widgets['XL'].values())
        input_row_idx += 1
        self.widgets['C'] = self._make_input_row(input_labelframe, input_row_idx, 'C', 'µF', list(UNIT_FACTORS['C'].keys()), is_optional=True)
        self.rc_widgets_ref.extend(self.widgets['C'].values())
        input_row_idx += 1
        self.widgets['XC'] = self._make_input_row(input_labelframe, input_row_idx, 'X\u1D9C', 'Ω', list(UNIT_FACTORS['X'].keys()), is_optional=True)
        ...
        self.btn_solve = ttk.Button(button_frame, text='Solve', command=self.solve, width=10)
        self.btn_solve.grid(row=0, column=0, padx=5)
        self.btn_clear = ttk.Button(button_frame, text='Clear', command=self.clear_inputs, width=10)
        self.btn_clear.grid(row=0, column=1, padx=5)
        self.btn_plot_wave = ttk.Button(button_frame, text='Plot Waveforms', command=self.plot_waveforms_action, state=tk.DISABLED, width=15)
        self.btn_plot_wave.grid(row=0, column=2, padx=5)
        # NEW Phasor Plot Button
        self.btn_plot_phasor = ttk.Button(button_frame, text='Plot Phasors', command=self.plot_phasors_action, state=tk.DISABLED, width=15)
        ...
        paned_window.grid(row=current_row, column=0, columnspan=3, sticky=(tk.W, tk.E, tk.N, tk.S), pady=(5, 5))
        ...
        self.text_output = scrolledtext.ScrolledText(results_frame, width=60, height=10, wrap=tk.WORD, relief=tk.SUNKEN, borderwidth=1, font=('Consolas', 9), state=tk.DISABLED)
        self.text_output.grid(row=0, column=0, sticky=(tk.W, tk.E, tk.N, tk.S))

        # --- Calculation Steps Area (Bottom Pane) ---
        steps_frame = ttk.LabelFrame(paned_window, text=" Calculation Steps ", padding=(10, 5))
        steps_frame.rowconfigure(0, weight=1)
        steps_frame.columnconfigure(0, weight=1)
        paned_window.add(steps_frame, weight=1) # Add to paned window

        self.text_calc_steps = scrolledtext.ScrolledText(steps_frame, width=60, height=8, wrap=tk.WORD, relief=tk.SUNKEN, borderwidth=1, font=('Consolas', 9), state=tk.DISABLED)
        self.text_calc_steps.grid(row=0, column=0, sticky=(tk.W, tk.E, tk.N, tk.S))


        # --- Status Bar ---
        self.status_var = tk.StringVar(value="Ready")
        status_bar = ttk.Label(self.frame, textvariable=self.status_var, relief=tk.SUNKEN, anchor=tk.W, style='StatusBar.TLabel')
        status_bar.grid(row=current_row, column=0, columnspan=3, sticky=(tk.W, tk.E), pady=(5,0))
    ...
    def _make_input_row(self, parent_frame: ttk.Widget, row: int, label_text: str, default_unit: str, units: list, is_optional: bool = False) -> Dict[str, ttk.Widget]:
        ...
        unit_combo = ttk.Combobox(parent_frame, values=units, width=5, state='readonly', justify='left')
    ...
    def _get_input_value(self, widget_key: str, component_type_key: str) -> Optional[float]:
        ...
        if not all([entry_widget, unit_widget, label_widget]):
             raise ValueError(f"Internal Error: Missing widget components for '{widget_key}'.")
        ...
        try:
            ...
            raise ValueError(f"Invalid numeric input for '{label_text}'. Please enter a number (e.g., 10.5 or 1.5e3).") from None

        try:
            unit = unit_widget.get()
            self.input_values[f"{widget_key}_unit"] = unit # Store unit used
            if component_type_key not in UNIT_FACTORS or unit not in UNIT_FACTORS[component_type_key]:
            ...
            return result_value
             ...
             raise ValueError(f"Invalid unit '{unit}' or component type '{component_type_key}' lookup.")
    ...
    def _toggle_fields(self, *args):
        ...
        if not is_rl:
            if 'L' in self.widgets: self.widgets['L']['entry'].delete(0, tk.END)
            if 'XL' in self.widgets: self.widgets['XL']['entry'].delete(0, tk.END)
        else:
            if 'C' in self.widgets: self.widgets['C']['entry'].delete(0, tk.END)
            if 'XC' in self.widgets: self.widgets['XC']['entry'].delete(0, tk.END)
    ...
    def _display_calculation_steps(self):
        ...
        def fmt(val, precision='4g'):
            ...
            if math.isinf(val): return "Infinity"
        ...
        if circuit_type == 'RL':
            ...
            steps.append(f"X_L = {fmt(xl_in) if xl_in is not None else 'Not Provided'} Ω")
        else:
            c_in = inp.get('C_si')
            xc_in = inp.get('XC_si')
            steps.append(f"C = {fmt(c_in, '4e') if c_in is not None else 'Not Provided'} F") # Use scientific for C
            steps.append(f"X_C = {fmt(xc_in) if xc_in is not None else 'Not Provided'} Ω")
        ...
        if omega_val is not None:
             if f_in is not None: # Calculated from input f
                 ...
                 steps.append(f"ω = 2π * f (derived) = 2π * {fmt(f_val)} = {fmt(omega_val)} rad/s")
        ...
        if react_val is not None:
            if circuit_type == 'RL':
                ...
                if xl_in is None and omega_val is not None and l_val is not None:
                     steps.append(f"X_L = ω * L = {fmt(omega_val)} * {fmt(l_val)} = {fmt(react_val)} Ω")
                elif l_in is None and omega_val is not None and react_val is not None:
                     steps.append(f"L = X_L / ω = {fmt(react_val)} / {fmt(omega_val)} = {fmt(l_val)} H")
                elif f_in is None and l_val is not None and react_val is not None:
                     if l_val != 0:
                         steps.append(f"ω = X_L / L = {fmt(react_val)} / {fmt(l_val)} = {fmt(omega_val)} rad/s")
                         steps.append(f"f = ω / 2π = {fmt(omega_val)} / 2π = {fmt(f_val)} Hz")
                     else: steps.append("Cannot derive f from X_L if L=0")
                ...
                if xc_in is None and omega_val is not None and c_val is not None:
                     if omega_val != 0 and c_val != 0:
                         steps.append(f"X_C = 1 / (ω * C) = 1 / ({fmt(omega_val)} * {fmt(c_val, '4e')}) = {fmt(react_val)} Ω")
                     elif omega_val == 0:
                          steps.append(f"X_C = Infinity (DC open circuit)")
                     else: steps.append("Cannot calculate X_C if C or ω is zero")
                     ...
                     if omega_val != 0 and react_val != 0 and not math.isinf(react_val):
                         steps.append(f"C = 1 / (ω * X_C) = 1 / ({fmt(omega_val)} * {fmt(react_val)}) = {fmt(c_val, '4e')} F")
                     else: steps.append("Cannot derive C if ω=0 or X_C=0 or X_C=inf")
                elif f_in is None and c_val is not None and react_val is not None:
                     if c_val != 0 and react_val != 0 and not math.isinf(react_val):
                         steps.append(f"ω = 1 / (C * X_C) = 1 / ({fmt(c_val, '4e')} * {fmt(react_val)}) = {fmt(omega_val)} rad/s")
                         steps.append(f"f = ω / 2π = {fmt(omega_val)} / 2π = {fmt(f_val)} Hz")
                     elif react_val == float('inf'):
                          steps.append("f = 0 Hz (from X_C = Infinity)")
                     else: steps.append("Cannot derive f if C=0 or X_C=0")
        ...
        if Z_val is not None and R_val is not None and X_val is not None:
             if Z_val == float('inf'):
                 ...
                 steps.append(f"Z = sqrt(R² + X²) = sqrt({fmt(R_val)}² + {fmt(X_val)}²) = {fmt(Z_val)} Ω")
                 # 4. Phase Angle
                 if phi_val is not None:
                      react_signed = X_val if circuit_type == 'RL' else -X_val
                      steps.append(f"φ = atan2(X_signed, R) = atan2({fmt(react_signed)}, {fmt(R_val)}) = {fmt(math.radians(phi_val), '4f')} rad")
        ...
        if I_rms_val is not None and V_rms_val is not None and Z_val is not None:
             if Z_val == float('inf'):
                 steps.append("I_rms = V_rms / Z = V_rms / Infinity = 0 A")
                  ...
                  steps.append(f"I_rms = V_rms / Z = {fmt(V_rms_val)} / 0 = Infinity A (Short circuit)")
             else:
                 steps.append(f"I_rms = V_rms / Z = {fmt(V_rms_val)} / {fmt(Z_val)} = {fmt(I_rms_val)} A")
        else:
             steps.append("I_rms = ?")
        ...
        if VR_rms_val is not None and I_rms_val is not None and R_val is not None:
             if I_rms_val == float('inf'):
                 steps.append(f"V_R_rms = I_rms * R = Infinity * {fmt(R_val)} = {'Infinity' if R_val > 0 else '0'} V")
             else:
                 steps.append(f"V_R_rms = I_rms * R = {fmt(I_rms_val)} * {fmt(R_val)} = {fmt(VR_rms_val)} V")
        else:
             steps.append("V_R_rms = ?")

        if VX_rms_val is not None and I_rms_val is not None and X_val is not None:
             if I_rms_val == float('inf'):
                  steps.append(f"V_X_rms = I_rms * X = Infinity * {fmt(X_val)} = {'Infinity' if X_val > 0 else '0'} V")
             else:
                  steps.append(f"V_X_rms = I_rms * X = {fmt(I_rms_val)} * {fmt(X_val)} = {fmt(VX_rms_val)} V")
        else:
             steps.append("V_X_rms = ?")

        # 7. Verification Check (Optional but good)
        if V_rms_val is not None and VR_rms_val is not None and VX_rms_val is not None \
           and not math.isinf(V_rms_val) and not math.isinf(VR_rms_val) and not math.isinf(VX_rms_val):
            check_val = math.hypot(VR_rms_val, VX_rms_val)
            steps.append("\n--- Verification ---")
            steps.append(f"Check: sqrt(V_R² + V_X²) = sqrt({fmt(VR_rms_val)}² + {fmt(VX_rms_val)}²)")
    ...
    def solve(self):
        ...
        try:
            ...
            circuit_type = self.circuit_var.get()
            if circuit_type == 'RL':
                self.calc_params = calculate_series_ac_circuit(V_rms, R, L, X_L, f, 'RL')
            else: # RC
                self.calc_params = calculate_series_ac_circuit(V_rms, R, C, X_C, f, 'RC')
            ...
            return
             ...
             self.text_calc_steps.config(state=tk.DISABLED)
             import traceback
             print("--- UNEXPECTED ERROR ---"); traceback.print_exc(); print("------------------------")
             return
        ...
        display_map = [
            ('V_rms', 'RMS Source Voltage (V\u209B)', 'V'), ('R', 'Resistance (R)', 'Ω'),
            ('f', 'Frequency (f)', 'Hz'), ('L', 'Inductance (L)', 'H'),
            ('C', 'Capacitance (C)', 'F'), ('X_L', 'Ind. Reactance (X\u2097)', 'Ω'),
            ('X_C', 'Cap. Reactance (X\u1D9C)', 'Ω'), ('omega', 'Angular Freq. (ω)', 'rad/s'),
            ('Z', 'Impedance Mag. (|Z|)', 'Ω'), ('phi', 'Impedance Phase (∠Z)', '°'),
            ('I_rms', 'RMS Current (I)', 'A'), ('I_peak', 'Peak Current (I_pk)', 'A'),
            ('V_peak', 'Peak Source Voltage (V\u209B_pk)', 'V'), ('V_rms_R', 'RMS Voltage (V\u1D63)', 'V'),
            ('V_rms_X', 'RMS Voltage (V\u2093)', 'V'),
        ]
        results_text = ""
        for key, label, unit in display_map:
            is_rl_key = key in ['L', 'X_L']; is_rc_key = key in ['C', 'X_C']
            if circuit_type == 'RL' and is_rc_key: continue
            if circuit_type == 'RC' and is_rl_key: continue
            if key in self.calc_params and self.calc_params[key] is not None:
                value = self.calc_params[key]
                if isinstance(value, (int, float)):
                    if value == float('inf'): formatted_value = "Infinity"
                    elif abs(value) > 1e7 or (abs(value) < 1e-4 and value != 0): formatted_value = f"{value:.4e}"
                    else: formatted_value = f"{value:.5g}"
        ...
        if self.calc_params.get('Z') is not None and self.calc_params.get('phi') is not None:
             Z_val = self.calc_params['Z']; phi_val = self.calc_params['phi']
             if Z_val == float('inf'): z_str = "Infinity"
             elif abs(Z_val) > 1e7 or (abs(Z_val) < 1e-4 and Z_val != 0): z_str = f"{Z_val:.4e}"
             else: z_str = f"{Z_val:.5g}"
        ...
        no_inf = self.calc_params.get('Z') != float('inf') and self.calc_params.get('I_rms') != float('inf')

        if no_inf and (is_ac or is_dc): # Waveforms can be plotted for AC or DC (if not open/short)
             self.btn_plot_wave.config(state=tk.NORMAL)
        else:
             self.btn_plot_wave.config(state=tk.DISABLED)

        if no_inf and is_ac: # Phasors typically only plotted for AC (if not open/short)
             self.btn_plot_phasor.config(state=tk.NORMAL)
        else:
             self.btn_plot_phasor.config(state=tk.DISABLED)

        if not no_inf and self.calc_params: # If calculation ran but resulted in open/short
    ...
    def plot_waveforms_action(self):
        ...
        if not self.calc_params or self.btn_plot_wave['state'] == tk.DISABLED:
            messagebox.showwarning('Plot Error', 'Cannot plot waveforms for the current results.')
            return
        try:
             ...
             messagebox.showerror('Plotting Error', f"Could not generate waveform plot:\n{e}")
             self.status_var.set("Plotting Error!")
        except Exception as e:
             messagebox.showerror('Plotting Error', f"Unexpected error during waveform plotting:\n{type(e).__name__}: {e}")
             self.status_var.set("Plotting Error!")
             import traceback; print("--- UNEXPECTED WAVE PLOT ERROR ---"); traceback.print_exc(); print("--------------------------------")
    ...
    def plot_phasors_action(self):
        ...
        if not self.calc_params or self.btn_plot_phasor['state'] == tk.DISABLED:
            messagebox.showwarning('Plot Error', 'Cannot plot phasors for the current results (Requires AC, non-open/short).')
            return
        try:
             ...
             messagebox.showerror('Plotting Error', f"Could not generate phasor plot:\n{e}")
             self.status_var.set("Plotting Error!")
        except Exception as e:
             messagebox.showerror('Plotting Error', f"Unexpected error during phasor plotting:\n{type(e).__name__}: {e}")
             self.status_var.set("Plotting Error!")
             import traceback; print("--- UNEXPECTED PHASOR PLOT ERROR ---"); traceback.print_exc(); print("----------------------------------")
...
if __name__ == "__main__":
    try:
        ...
        try:
            ...
            print(f"Could not show Tkinter error message box: {tk_e}")